...
```python
sns.scatterplot(x='ZCTA5',y='Composite_Score_Weighted',hue='Risk_Label',palette='seismic',data=df)
...
```
    ...
    [<matplotlib.lines.Line2D at 0x30bd96e70>]
...
```python
import sys
sys.path.append('/Users/bayowaonabajo/Downloads')
...
```
    ...
    File ~/Downloads/svm_margin_plot.py:27, in plot_svm_boundary(model, X, y)
         25 YY, XX = np.meshgrid(yy, xx)
         26 xy = np.vstack([XX.ravel(), YY.ravel()]).T
    ---> 27 Z = model.decision_function(xy).reshape(XX.shape)
         29 # plot decision boundary and margins
         30 ax.contour(XX, YY, Z, colors='k', levels=[-1, 0, 1], alpha=0.5,
         31            linestyles=['--', '-', '--'])


    File /opt/anaconda3/lib/python3.12/site-packages/sklearn/svm/_base.py:787, in BaseSVC.decision_function(self, X)
        760 def decision_function(self, X):
        761     """Evaluate the decision function for the samples in X.
        762 
        763     Parameters
       (...)
        785     transformation of ovo decision function.
        786     """
    --> 787     dec = self._decision_function(X)
        788     if self.decision_function_shape == "ovr" and len(self.classes_) > 2:
        789         return _ovr_decision_function(dec < 0, -dec, len(self.classes_))


    File /opt/anaconda3/lib/python3.12/site-packages/sklearn/svm/_base.py:536, in BaseLibSVM._decision_function(self, X)
        522 """Evaluates the decision function for the samples in X.
        523 
        524 Parameters
       (...)
        532     in the model.
        533 """
        534 # NOTE: _validate_for_predict contains check for is_fitted
        535 # hence must be placed before any other attributes are used.
    --> 536 X = self._validate_for_predict(X)
        537 X = self._compute_kernel(X)
        539 if self._sparse:


    File /opt/anaconda3/lib/python3.12/site-packages/sklearn/svm/_base.py:614, in BaseLibSVM._validate_for_predict(self, X)
        611 check_is_fitted(self)
        613 if not callable(self.kernel):
    --> 614     X = validate_data(
        615         self,
        616         X,
        617         accept_sparse="csr",
        618         dtype=np.float64,
        619         order="C",
        620         accept_large_sparse=False,
        621         reset=False,
        622     )
        624 if self._sparse and not sp.issparse(X):
        625     X = sp.csr_matrix(X)


    File /opt/anaconda3/lib/python3.12/site-packages/sklearn/utils/validation.py:2965, in validate_data(_estimator, X, y, reset, validate_separately, skip_check_array, **check_params)
       2962     out = X, y
    ...
    File /opt/anaconda3/lib/python3.12/site-packages/sklearn/utils/validation.py:2829, in _check_n_features(estimator, X, reset)
       2826     return
...
```
    ...
    Cell In[245], line 3
          1 model = SVC(kernel='poly', C=1,degree=1)
          2 model.fit(X, y)
    ----> 3 plot_svm_boundary(model,X,y)
...
```
    ...
    Cell In[247], line 3
          1 model = SVC(kernel='poly', C=1,degree=2)
          2 model.fit(X, y)
    ----> 3 plot_svm_boundary(model,X,y)
...
#sk-container-id-7 a.estimator_doc_link.fitted:hover {
  ...
  background-color: var(--sklearn-color-fitted-level-3);
...
</style><div id="sk-container-id-7" class="sk-top-container"><div class="sk-text-repr-fallback"><pre>GridSearchCV(estimator=SVC(),
             ...
             param_grid={&#x27;C&#x27;: [0.01, 0.1, 1], &#x27;kernel&#x27;: [&#x27;linear&#x27;, &#x27;rbf&#x27;]})</pre></div> </div></div><div class="sk-parallel"><div class="sk-parallel-item"><div class="sk-item"><div class="sk-label-container"><div class="sk-label fitted sk-toggleable"><input class="sk-toggleable__control sk-hidden--visually" id="sk-estimator-id-13" type="checkbox" ><label for="sk-estimator-id-13" class="sk-toggleable__label fitted sk-toggleable__label-arrow"><div><div>best_estimator_: SVC</div></div></label><div class="sk-toggleable__content fitted"><pre>SVC(C=0.1, kernel=&#x27;linear&#x27;)</pre></div> </div></div><div class="sk-serial"><div class="sk-item"><div class="sk-estimator fitted sk-toggleable"><input class="sk-toggleable__control sk-hidden--visually" id="sk-estimator-id-14" type="checkbox" ><label for="sk-estimator-id-14" class="sk-toggleable__label fitted sk-toggleable__label-arrow"><div><div>SVC</div></div><div><a class="sk-estimator-doc-link fitted" rel="noreferrer" target="_blank" href="https://scikit-learn.org/1.6/modules/generated/sklearn.svm.SVC.html">?<span>Documentation for SVC</span></a></div></label><div class="sk-toggleable__content fitted"><pre>SVC(C=0.1, kernel=&#x27;linear&#x27;)</pre></div> </div></div></div></div></div></div></div></div></div>
